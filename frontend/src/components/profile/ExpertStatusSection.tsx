...
import { RocketOutlined, CalendarOutlined, CopyOutlined, BarChartOutlined, BookOutlined, CalendarFilled } from '@ant-design/icons';
  ...
  return (
    <>
      ...
      <div className="section-card" style={{ padding: 0, overflow: 'hidden' }}>
        <div style={{ padding: '24px 32px', borderBottom: '1px solid #f2f2f2' }}>
          <h2 className="section-title" style={{ margin: 0 }}>🏠 Кабинет эксперта</h2>
        </div>
        <div style={{ padding: '20px 32px', display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 12 }}>
          <button className="cabinet-item" onClick={() => navigate('/expert-calendar')}>
            <CalendarFilled style={{ fontSize: 24, color: '#5856d6' }} />
            <span>Календарь</span>
          </button>
          <button className="cabinet-item" onClick={() => navigate('/articles/my')}>
            <BookOutlined style={{ fontSize: 24, color: '#ff9500' }} />
            <span>Мои статьи</span>
          </button>
          <button className="cabinet-item" onClick={() => navigate('/expert-landing')}>
            <BarChartOutlined style={{ fontSize: 24, color: '#34c759' }} />
            <span>Статистика</span>
          </button>
        </div>
      </div>

      {/* Статус + Реферальная программа */}
      <div className="section-card" style={{ padding: 0, overflow: 'hidden' }}>
        {/* Статус */}
        <div style={{
